...
class ImageCanvas(QLabel):
    def __init__(self):
        ...
        self.image = None
        self.q_image = None  # Use QImage for drawing
    ...
    def wheelEvent(self, event):
        ...
        # Calculate scene position before zoom
        scene_pos_before = (view_pos - self.pan_offset) / (self.zoom_level / zoom_factor if event.angleDelta().y() > 0 else self.zoom_level * zoom_factor)

        # Calculate scene position after zoom
        scene_pos_after = (view_pos - self.pan_offset) / self.zoom_level

        # Adjust pan offset to keep mouse position stable
        self.pan_offset += (scene_pos_after - scene_pos_before) * self.zoom_level
    ...
    def mousePressEvent(self, event):
        """Handle mouse press for drawing, panning, dragging, and resizing."""
        ...
        if event.button() == Qt.LeftButton:
            ...
            if modifiers == Qt.AltModifier:
                bbox, handle = self.find_closest_bbox_for_resize(pos)
                if bbox and handle:
                    self.resizing_bbox = bbox
                    self.resize_start_pos = pos
                    self.resize_original_rect = copy.deepcopy(bbox.rect)
                    self.resize_handle = handle
                    self.update_display()
                    return

            # Drag mode with Ctrl key
            elif modifiers == Qt.ControlModifier:
                # Find bbox to drag by checking from top-most (last in list)
                for bbox in reversed(self.bounding_boxes):
    ...
    def mouseMoveEvent(self, event):
        """Handle mouse move for drawing, panning, dragging, and resizing."""
        # Handle panning
        if self.panning:
    ...
    def mouseReleaseEvent(self, event):
        """Handle mouse release for drawing, panning, dragging, and resizing."""
        # Stop panning
        ...
        if event.button() == Qt.LeftButton:
            if self.drawing:
                ...
                self.update_display()
        elif event.button() == Qt.RightButton: # Right-click for deletion
            pos = self.get_image_coordinates(event.pos())
            if pos is None:
                return

            # Find the bounding box that was right-clicked
            bbox_to_delete = None
            delete_index = -1
            for i, bbox in enumerate(reversed(self.bounding_boxes)):
                x1, y1, x2, y2 = bbox.rect
                if x1 <= pos[0] <= x2 and y1 <= pos[1] <= y2:
                    bbox_to_delete = bbox
                    delete_index = len(self.bounding_boxes) - 1 - i # Get original index
                    break

            if bbox_to_delete:
                reply = QMessageBox.question(self, "바운딩 박스 삭제",
                                             f"'{bbox_to_delete.class_name}' 바운딩 박스를 삭제하시겠습니까?",
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if reply == QMessageBox.Yes:
                    cmd = DeleteBBoxCommand(self, bbox_to_delete, delete_index)
                    cmd.execute()
                    self.execute_command(cmd)
                    self.update_display()
            
    def get_image_coordinates(self, view_pos):
        if self.q_image is None:
            return None

        # Transform view coordinates to scene (image) coordinates
        ...
        scene_pos = inverted_transform.map(view_pos)

        # Check bounds
        if 0 <= scene_pos.x() < self.q_image.width() and 0 <= scene_pos.y() < self.q_image.height():
    ...
    def paintEvent(self, event):
        ...
        if self.q_image:
            ...
            for bbox in self.bounding_boxes:
                ...
                painter.drawText(x1, y1 - 5, bbox.class_name)

            # Draw drawing rectangle
            if self.drawing and self.start_point and self.end_point:
                pen = QPen(QColor(255, 255, 0), 2, Qt.DashLine) # Yellow dashed line
                painter.setPen(pen)
                rect = QRect(QPoint(*self.start_point), QPoint(*self.end_point)).normalized()
                painter.drawRect(rect)
                
            # Draw guide lines
            if self.show_guides and self.mouse_pos:
                pen = QPen(QColor(*self.guide_color), 1, Qt.DashLine)
                painter.setPen(pen)
                
                # Horizontal line
                painter.drawLine(0, self.mouse_pos[1], self.q_image.width(), self.mouse_pos[1])
                # Vertical line
                painter.drawLine(self.mouse_pos[0], 0, self.mouse_pos[0], self.q_image.height())
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.image_canvas.setFocus() # Set initial focus to canvas
        ...
        class_layout = QVBoxLayout(scroll_widget)
        
        # Add "Select All" checkbox
        self.select_all_checkbox = QCheckBox("Select All")
        self.select_all_checkbox.stateChanged.connect(self.toggle_all_classes)
        self.select_all_checkbox.setVisible(False)  # Initially hidden
        class_layout.addWidget(self.select_all_checkbox)

        # Add a separator
        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setFrameShadow(QFrame.Sunken)
        separator.setVisible(False) # Initially hidden
        class_layout.addWidget(separator)
        self.class_separator = separator
        ...
        class_group_layout.addWidget(scroll_area)
        ...
        class_group_layout.addWidget(self.yolo_world_detect_btn)
        
        class_group.setLayout(class_group_layout)
    ...
    def load_current_image(self):
        if 0 <= self.current_image_index < len(self.image_files):
            ...
            if self.image_canvas.set_image(image_path):
                ...
                self.image_canvas.setFocus() # Ensure canvas has focus for key events
    ...
    def toggle_yolo_world_mode(self, checked):
        ...
        if checked:
            ...
            self.class_separator.setVisible(True)
            ...
            self.class_separator.setVisible(False)
            for checkbox in self.class_checkboxes:
                checkbox.setVisible(False)
    ...
    def toggle_all_classes(self, state):
        """Select or deselect all class checkboxes"""
